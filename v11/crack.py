...
class ngram_score:
    def __init__(self, ngramfile, sep=' '):
        ...
        self.ngramsL = [ngram for ngram, freq in self.ngrams.items()]
    ...
    def score(self, text):
        ...
        for ngram, freq in self.ngrams.items():
            ...
            # too slow
            # score += self.np.char.count(text, ngram) * freq
...
def crack(text):
    ...
    def _crackCeaser(text):
        for i in range(1, 27):
            ...
            if de:
                allCeaser[str(i)] = {'score':score.score(de), 'deciphered':de, 'key':i}
    ...
    def _crackRailfence(text, maxKey=100):
        for i in range(1, maxKey + 1):
            ...
            if de:
                allRailfence[str(i)] = {'score':score.score(de), 'deciphered':de, 'key':i}
        ...
        for a in range(1, maxKey + 1):
            for b in range(1, maxKey + 1):
                try:
                    ...
                    pass
                if de:
                    allAffine[f'{str(a)},{str(b)}'] = {'score':score.score(de), 'deciphered':de, 'key':a, 'key2':b}
